...
constexpr const char *CHAMBER_LIGHT_ON =
    R"({"system":{"sequence_id":"0","command":"ledctrl","led_node":"chamber_light","led_mode":"on","led_on_time":500,"led_off_time":500,"loop_times":0,"interval_time":0}})";
constexpr const char *CHAMBER_LIGHT_OFF =
    R"({"system":{"sequence_id":"0","command":"ledctrl","led_node":"chamber_light","led_mode":"off","led_on_time":500,"led_off_time":500,"loop_times":0,"interval_time":0}})";
constexpr const char *CHAMBER_LIGHT_2_ON =
    R"({"system":{"sequence_id":"0","command":"ledctrl","led_node":"chamber_light2","led_mode":"on","led_on_time":500,"led_off_time":500,"loop_times":0,"interval_time":0}})";
constexpr const char *CHAMBER_LIGHT_2_OFF =
    R"({"system":{"sequence_id":"0","command":"ledctrl","led_node":"chamber_light2","led_mode":"off","led_on_time":500,"led_off_time":500,"loop_times":0,"interval_time":0}})";

constexpr const char *HEATBED_LIGHT_ON =
    R"({"system":{"sequence_id":"0","command":"ledctrl","led_node":"heatbed_light","led_mode":"on","led_on_time":0,"led_off_time":0,"loop_times":0,"interval_time":0}})";
constexpr const char *HEATBED_LIGHT_OFF =
    R"({"system":{"sequence_id":"0","command":"ledctrl","led_node":"heatbed_light","led_mode":"off","led_on_time":0,"led_off_time":0,"loop_times":0,"interval_time":0}})";

constexpr const char *SPEED_PROFILE_TEMPLATE =
    R"({"print":{"sequence_id":"0","command":"print_speed","param":""}})";
...
constexpr const char *SEND_GCODE_TEMPLATE =
    R"({"print":{"sequence_id":"0","command":"gcode_line","param":""}})";
...
constexpr const char *SWITCH_AMS_TEMPLATE =
    R"({"print":{"command":"ams_change_filament","sequence_id":"0","target":255,"curr_temp":0,"tar_temp":0}})";

constexpr const char *AMS_FILAMENT_SETTING_TEMPLATE =
    R"({"print":{"sequence_id":"0","command":"ams_filament_setting","ams_id":0,"tray_id":0,"tray_info_idx":"","tray_color":"000000FF","nozzle_temp_min":0,"nozzle_temp_max":0,"tray_type":"PLA"}})";
...
// X1 only currently
constexpr const char *GET_ACCESSORIES =
    R"({"system":{"sequence_id":"0","command":"get_accessories","accessory_type":"none"}})";

// A1 and H2D only
constexpr const char *PROMPT_SOUND_ENABLE =
    R"({"print":{"sequence_id":"0","command":"print_option","sound_enable":true}})";
constexpr const char *PROMPT_SOUND_DISABLE =
    R"({"print":{"sequence_id":"0","command":"print_option","sound_enable":false}})";

// H2D only
constexpr const char *BUZZER_SET_SILENT =
    R"({"print":{"sequence_id":"0","command":"buzzer_ctrl","mode":0,"reason":""}})";
constexpr const char *BUZZER_SET_ALARM =
    R"({"print":{"sequence_id":"0","command":"buzzer_ctrl","mode":1,"reason":""}})";
constexpr const char *BUZZER_SET_BEEPING =
    R"({"print":{"sequence_id":"0","command":"buzzer_ctrl","mode":2,"reason":""}})";
...
inline std::string SwitchAmsCmd(int target, int curr_temp, int tar_temp) {
    ...
    oss << R"({"print":{"command":"ams_change_filament","sequence_id":"0","target":)" << target
        << R"(,"curr_temp":)" << curr_temp << R"(,"tar_temp":)" << tar_temp << "}}";
    return oss.str();
}

// AMS_FILAMENT_SETTING_TEMPLATE: ams_id, tray_id, tray_info_idx, tray_color, nozzle_temp_min, nozzle_temp_max, tray_type
inline std::string AmsFilamentSettingCmd(int ams_id, int tray_id, const std::string &tray_info_idx,
                                         const std::string &tray_color, int nozzle_temp_min,
                                         int nozzle_temp_max, const std::string &tray_type) {
    std::ostringstream oss;
    oss << R"({"print":{"sequence_id":"0","command":"ams_filament_setting","ams_id":)" << ams_id
        << R"(,"tray_id":)" << tray_id << R"(,"tray_info_idx":")" << tray_info_idx
        << R"(","tray_color":")" << tray_color << R"(","nozzle_temp_min":)" << nozzle_temp_min
        << R"(,"nozzle_temp_max":)" << nozzle_temp_max << R"(,"tray_type":")" << tray_type
        << R"("}})";
    return oss.str();
}
